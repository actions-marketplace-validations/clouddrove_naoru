...
├── vitest.config.js
├── src/
│   ├── index.js            # orchestrator, error-safe entry
...
async function run() {
  ...
  const cfg = resolveProvider({ provider, apiKey, baseURL, model })
  const prompt = buildPrompt({ jobName, logs, diff, files })
  const raw = cfg.kind === 'anthropic' ? await anthropicDiagnose(cfg, prompt) : await openaiDiagnose(cfg, prompt)
  const d = normalize(raw)

  const url = await upsertComment(octokit, { owner, repo, prNumber, body: toMarkdown(jobName, d) })

  core.setOutput('root-cause', d.rootCause)
  core.setOutput('confidence', d.confidence)
...
git commit -m "feat: orchestrator entry + bundled dist"
```
      ...
      - name: dist freshness
        run: |
          ...
          if ! git diff --exit-code dist; then
            echo "::error::dist/ is stale. Run 'npm run build' and commit." && exit 1
          fi
```
...
Marketplace listing: hero line + meaning of 直る, 30-second quickstart (the `examples/usage.yml` snippet), provider table (anthropic/openai/openrouter/xai/groq/custom with base-urls), inputs/outputs tables (mirror `action.yml`), how the sticky comment works, cost note (sonnet default), fail-safe note, Apache-2.0.

- [ ] **Step 3: Commit**

```bash
git add README.md examples/usage.yml
git commit -m "docs: readme marketplace listing and usage example"
```
...
- **Spec coverage:** name/meaning (T1, T12), composite action + node20 (T7,T8), multi-provider abstraction (T4,T5), sticky comment (T3,T6), structured output (T2,T5), log truncation (T6), fail-safe (T7), repo CI + dist freshness (T9), dogfood (T10), release/major-tag (T11), marketplace README (T12), Apache-2.0 (T1). All covered.
- **Type consistency:** `diagnose(cfg, prompt)` signature identical across both providers; `normalize`/`toMarkdown`/`MARKER` shared from `parse.js`; `DIAGNOSIS_SCHEMA` shared from `prompt.js`.
